...
如果需要增加新的全局样式，先在`__assets`目录下创建新的css文件，然后在`__document.html`文件中引入新的css文件。

如果需要增加新的全局脚本或是引用外部库文件，比如`jQuery`等，先在`__assets`目录下创建新的js文件，然后在`__document.html`文件中引入新的js文件。
...
- 在后端全局数据配置文件`__data.json`中配置的数据，在前端全局变量`__sui_data`中读取。
- 前端调用后端API：
...
// Component utilities
//在页面中查找component，返回值component实例
